...
export default function ThermModal(props: ThermModalProps) {
	...
	const { degreesFormat, thermInterval, use24Hour } = props.settings;
	...
	return (
		<div className="w-full bg-gray-300 z-10 absolute top-0 left-0">
			<div className="h-screen flex flex-row justify-around lg:justify-center items-center p-5">
				<div className="flex flex-col lg:w-1/4 w-5/12 bg-blue-500 bg-opacity-75 rounded-lg p-5 shadow-xl justify-around lg:mr-10">
					...
					<div className="flex flex-row justify-around mt-5">
						{timelines.map((timeline, index) => (
							<button key={index} className={thermInterval === intervals[index] ? activeModalBtn : inactiveModalBtn} onClick={() => setSettings(props.settings, 'thermInterval', intervals[index])}>
								{timeline}
							</button>
						))}
					</div>
				...
				<ThermPanelChild thermostat={props.therm} updateModalDisplay={() => { }} width="1/2" degreesFormat={degreesFormat} use24Hour={use24Hour} past={pastData(props.therm)} showThermModal={props.showThermModal} isModal={true} thermInterval={thermInterval} />
			</div>
		</div>
	);
}
