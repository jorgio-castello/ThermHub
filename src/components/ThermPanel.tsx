    ...
    return (
        <>
            ...
            <div className="flex flex-row p-5 rounded-lg shadow-lg bg-blue-100 bg-opacity-75 mb-10 justify-start border border-solid border-indigo-200">
                { props.thermostatData.map((thermostat, index) => <ThermPanelChild thermostat={thermostat} width="1/4" key={index} id={index} updateModalDisplay={props.expandThermPanel} degreesFormat={props.degreesFormat} use24Hour={props.use24Hour} past={pastData(thermostat)} />) }
            </div>
        </>
    );
}
